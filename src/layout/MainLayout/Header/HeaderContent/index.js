...
const HeaderContent = () => {
  ...
  const getAdminSelectedEmployerUserLabel = (user) => {
    ...
    else if (user?.firstName) fullName = user?.firstName;
    else if (user?.lastName) fullName = user?.lastName;
  ...
  }, [keycloak?.tokenParsed?.roles, keycloak?.idToken, keycloak, dispatch]);
