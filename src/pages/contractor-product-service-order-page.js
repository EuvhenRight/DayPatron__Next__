...
const ContractorProductServiceOrderPage = () => {
  ...
  const handleApproveConfirmClick = async () => {
    let response = await fetch(process.env.REACT_APP_JOBMARKET_API_BASE_URL + '/products/orders/' + order.id + '/contractor-service-orders/admin-approvals',
      {
        method: 'PUT',
        headers: {
          'Authorization': 'Bearer ' + keycloak.idToken,
  ...
  return (
    <MainCard title="Talent Solution Service Order">
