...
const ContractorSubscriptionServiceOrderPage = () => {
  ...
  const handleApproveConfirmClick = async () => {
    let response = await fetch(process.env.REACT_APP_JOBMARKET_API_BASE_URL + '/subscriptions/orders/' + order.id + '/contractor-service-orders/contractor-approvals',
      {
        method: 'PUT',
        headers: {
          'Authorization': 'Bearer ' + keycloak.idToken,
  ...
  return (
    <MainCard title="Subscription Service Order">
