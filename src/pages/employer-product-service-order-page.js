...
const EmployerProductServiceOrderPage = () => {
  ...
  const handleApproveConfirmClick = async () => {
    let response = await fetch(process.env.REACT_APP_JOBMARKET_API_BASE_URL + '/products/orders/' + order.id + '/employer-service-orders/' + role + '-approvals',
      {
        method: 'PUT',
        headers: {
          'Authorization': 'Bearer ' + keycloak.idToken,
  ...
  return (
    <MainCard title="Company Solution Service Order">
...
export default EmployerProductServiceOrderPage;