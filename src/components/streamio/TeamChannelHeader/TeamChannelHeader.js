...
import { Avatar, AvatarGroup, Stack, Typography, Divider } from '@mui/material';
...
export const TeamChannelHeader = ({ setIsEditing, onShowChannelSelector, isChannelSelectorVisible }) => {
  ...
  const getChannelTitle = () => {
    ...
    if (allMembers.length === 1) members = allMembers;

    let titleText = members
      ?.map((x) => {
        let result = x?.user?.name || x?.user?.id;
        if (x.user.id === client.userID) result += ' (Me)';
        return result;
      })
      .join(', ');
    ...
    return (
      <>
        ...
        <Typography
          sx={{ textWrap: 'nowrap', fontWeight: 'bold', marginLeft: '5px !important', overflowX: 'hidden', textOverflow: 'ellipsis' }}
        >
          {titleText}
        </Typography>
      </>
      ...
      <Stack direction="row" justifyContent="space-between" sx={{ padding: '10px 20px 10px 20px' }} alignItems="center">
        ...
        <Stack direction="row" spacing={1} alignItems="center">
          <IconButton edge="end" aria-label="comments" color="secondary" onClick={handleMembersClick}>
            ...
            <Typography>{allMembers?.length}</Typography>
          </IconButton>
        </Stack>
      </Stack>
      <Divider />
