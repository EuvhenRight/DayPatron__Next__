...
const Variants: React.FC<VariantsProps> = ({
	...
	const handleVolumeClick = useCallback(
		...
		[currentIndex, setCurrentIndex, setAnimate]
	)
			...
			<ul className='flex gap-5 items-center my-2' onBlur={handleContainerBlur}>
				...
				{variantsProduct.map((variant, index) => {
					return (
						<li
							// FOCUS VOLUME BUTTON
							className='cursor-pointer border border-black px-2 py-1 focus:ring-2 focus:ring-black focus:outline-none snap-center text-lg'
							key={index}
							tabIndex={0}
